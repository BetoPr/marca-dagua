...
function renderCard(p) {
  ...
  // Detalhe (prompt.html) chega na Fase 3 - por enquanto so o botao Copiar e interativo
  card.innerHTML = `
...
function setupListeners() {
  ...
  $('grid').addEventListener('click', async e => {
    ...
    // Click fora do botao Copiar (na imagem ou body) avisa que detalhe vem em breve
    const card = e.target.closest('.gal-card');
    if (card) {
      e.preventDefault();
      showToast('Pagina de detalhe chega em breve');
    }
  });
