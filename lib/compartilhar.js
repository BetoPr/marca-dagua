...
function renderSummary() {
  const d = state.data;
  // Resolve src do preview seguindo prioridade:
  // 1) URL vinda de ?cover= ou modo edit
  // 2) data URL ja gerada no step 3 (coverPreviewDataUrl)
  // 3) fallback: object URL no momento
  let coverSrc = d.coverUrl || d.coverPreviewDataUrl;
  if (!coverSrc && d.coverFile) {
    try { coverSrc = URL.createObjectURL(d.coverFile); } catch {}
  }
  // Se ainda assim falhou mas o <img> do step 3 tem src valida, usa direto
  if (!coverSrc) {
    const previewImg = document.getElementById('coverPreview');
    const ps = previewImg?.getAttribute('src');
    if (ps && ps !== '' && ps !== '#') coverSrc = ps;
  }
  const cover = coverSrc ? `<img src="${coverSrc}" alt="" onerror="this.parentElement.innerHTML='<div class=\\'share-summary-noimg\\'>Erro ao carregar</div>'">` : `<div class="share-summary-noimg"><i data-lucide="image-off"></i> Sem capa</div>`;
...
function setupCoverUpload() {
  ...
  function setFile(file) {
    ...
    reader.onload = () => {
      ...
      preview.src = dataUrl;
      state.data.coverPreviewDataUrl = dataUrl; // cache pra renderSummary
      empty.hidden = true;
...
/**
 * Baixa cover de URL externa (ex: signed URL do histórico) como File,
 * pra ser re-uploadada no save pro bucket público prompt-covers.
 * Atualiza preview imediato. Se fetch falhar (CORS), fallback usa URL direta.
 */
async function fetchCoverAsFile(url) {
  const preview = $('coverPreview');
  const empty = $('dropEmpty');
  const previewWrap = $('dropPreview');
  ...
  try {
    ...
    if (blob.size > 10 * 1024 * 1024) {
      if (status) status.textContent = 'Imagem maior que 10MB — usando URL original';
      state.data.coverUrl = url;
      preview.src = url; empty.hidden = true; previewWrap.hidden = false;
      return;
    }
    const ext = (blob.type.split('/')[1] || 'jpg').replace('jpeg', 'jpg');
    const file = new File([blob], `seed-cover.${ext}`, { type: blob.type || 'image/jpeg' });
    state.data.coverFile = file;
    state.data.coverUrl = null; // será setado no save após upload
    ...
    reader.onload = () => {
      ...
      empty.hidden = true;
      previewWrap.hidden = false;
      if (status) status.textContent = '✓ Capa carregada';
    };
    reader.readAsDataURL(blob);
  } catch (err) {
    // CORS ou URL inválida → fallback usa URL direta (limitação: pode expirar)
    console.warn('fetchCoverAsFile falhou, usando URL direta:', err);
    state.data.coverUrl = url;
    preview.src = url;
    empty.hidden = true;
    previewWrap.hidden = false;
    if (status) status.textContent = '⚠ Fallback: usando URL original (pode expirar)';
  }
}
...
async function init() {
  ...
  // Cover: ?cover=<url> — baixa a imagem como File pra re-uploadar no save
  // (a URL original pode ser signed URL temporária do bucket privado prompt-images)
  const seedCover = params.get('cover');
  if (seedCover) {
    setTimeout(() => fetchCoverAsFile(seedCover), 0);
  }
